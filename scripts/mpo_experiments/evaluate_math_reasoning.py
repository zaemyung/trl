...
def compute_acc(path):
    ...
    for gen in generations:
        gold_answer = query_to_gold_answer[gen["query"]]
        pred_answer = extract_boxed_content(gen["model_response"])
        equiv = is_equiv(gold_answer, pred_answer, verbose=False)
        if equiv:
            corrects += 1

    print(f"{corrects}/{len(generations)} = {corrects / len(generations) * 100}\n\n")
