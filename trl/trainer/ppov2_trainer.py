...
from ..trainer.utils import (
    ...
    generate,
    get_reward,
    get_reward_from_prompt_based_rm,
    prepare_deepspeed,
    print_rich_table,
    truncate_response,
)
from ..extras.prompt_based_reward_model import Scorer
...
class PolicyAndValueWrapper(nn.Module):
    def __init__(self, policy, value_model) -> None:
        ...
        self.critic_backbone = value_model.pretrained_model
    ...
    def forward(self, **kwargs):
        ...
        logits = self.value_model.v_head(output.hidden_states[-1])
...
class PPOv2Trainer(Trainer):
    def __init__(
        ...
        for module in [policy, ref_policy, value_model, reward_model]:
            if isinstance(reward_model, Scorer):
                continue
    ...
    def save_model(
        ...
        if (
            not _internal_call
        ):  # `push_to_hub` already swaps out the self.model with policy
            self.backup_model = self.model
            self.model = self.accelerator.unwrap_model(
                self.model
            ).policy  # save only the policy
    ...
    def train(self):
        ...
        for update in tqdm(
            ...
            with torch.no_grad():
                ...
                with unwrap_model_for_generation(
                    ...
                    for i in range(
                        ...
                        unwrapped_value_model = self.accelerator.unwrap_model(
                            model
                        ).value_model
                        ...
                        detokenized_query = tokenizer.batch_decode(
                            query, skip_special_tokens=True
                        )
                        detokenized_response = tokenizer.batch_decode(
                            postprocessed_response
                        )

                        if isinstance(reward_model, Scorer):
                            score = get_reward_from_prompt_based_rm(
                                reward_model, detokenized_query, detokenized_response
                            )
            ...
            for ppo_epoch_idx in tqdm(
                range(args.num_ppo_epochs),
                total=args.num_ppo_epochs,
                desc="PPO Training",
            ):
    ...
    def generate_completions(self, sampling: bool = False):
        ...
        for batch in self.eval_dataloader:
            ...
            with torch.no_grad():
                ...
                detokenized_response = tokenizer.batch_decode(postprocessed_response)
                if isinstance(self.reward_model, Scorer):
                    score = get_reward_from_prompt_based_rm(
                        self.reward_model, detokenized_query, detokenized_response
                    )
                    score = torch.tensor(score, device=self.accelerator.device)
                    table["score"].extend(
