...
class MPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.meta_reward_model_address = meta_reward_model_address

        # Create directories and copy prompts
        self.experiment_directory = self.args.output_dir
        ...
        if (
            ...
            and self.args.wandb_entity is not None
            and self.args.wandb_project is not None
            and self.args.exp_name is not None
        ):
            wandb.init(entity=self.args.wandb_entity, project=self.args.wandb_project, name=self.args.exp_name)
    ...
    def train(self):
        ...
        for update in tqdm(
            ...
            for ppo_epoch_idx in tqdm(range(args.num_ppo_epochs), desc="PPO Training..", total=args.num_ppo_epochs):
            ...
            if update > 0 and update % args.num_mpo_interval == 0:
                ...
                for key in gatherings:
                    gatherings[key].clear()

                self.accelerator.wait_for_everyone()

                if self.accelerator.is_main_process:
                    evaluation_logs = self.meta_reward_model.meta_evaluate_and_update(
                        batch_index=update,
                        return_evaluations=True,
                    ...
                    with open(evaluation_logs_path, "w") as f:
                        f.write(f"{json.dumps(evaluation_logs, indent=4)}")
